@extends('admin.app')
@section('title') {{ isset($pageTitle)?$pageTitle:'Floors' }} @endsection
@push('styles')
    ...
    <div class="row">
        <div class="col-12 text-right">
            <a type="button" class="btn btn-info btn-min-width mr-1 mb-1"
               href="{{route('crm.floors.create')}}"><i
...
@push('scripts')
    ...
    <script>
        $('#floors-table').on('click', '.btn-delete[data-remote]', function (e) {
            e.preventDefault();
            $.ajaxSetup({
                headers: {
            ...
            if (confirm('Are you sure you want to delete this row?')) {
                $.ajax({
                    ...
                    $('#floors-table').DataTable().draw(false);
