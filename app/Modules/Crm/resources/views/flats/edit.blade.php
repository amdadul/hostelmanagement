    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title" id="basic-layout-card-center">Edit a Flat</h4>
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="select2 form-control @error('building_name') is-invalid @enderror" id="building_name" name="building_name" >
                                        ...
                                        @foreach($buildings as $building)
                                            @if($building->id == old('building_name')?old('building_name'):$data->floor->building_id)
                            ...
                            <div class="form-group">
                                ...
                                <select class="select2 form-control @error('floor_name') is-invalid @enderror" id="floor_name" name="floor_name" >
                                    ...
                                    @foreach($floors as $floor)
                                        @if($floor->id == old('floor_name')?old('floor_name'):$data->floor_id)
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="flat_name" class="form-control @error('flat_name') is-invalid @enderror"
                                          placeholder="Flat Name" name="flat_name">{{old('flat_name')?old('flat_name'):$data->name}}</textarea>
                                @error('flat_name')
                                <div class="help-block text-danger">{{ $message }} </div> @enderror
                            </div>
...
@push('scripts')
    ...
    function ajaxSave() {
        ...
        $.ajax({
            url: "{{ route('crm.flats.update',$data->id) }}",
            type: 'post',
            dataType: "json",
            cache: false,
            data: $('form').serialize(),
