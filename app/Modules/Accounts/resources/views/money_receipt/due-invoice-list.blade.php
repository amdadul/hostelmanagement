                ...
                <table class="table table-de mb-0" id="table-data-list">
                    ...
                    @if(!$data->isEmpty())
                        @foreach($data as $key => $dt)
                            <?php
                            $mrAmount = App\Modules\Accounts\Models\MoneyReceipt::totalMrAmountOfInvoice($dt->id);
                            $returnAmount = 0;
                            $totalMrWithReturn = $mrAmount + $returnAmount;
                            $due_amount = $dt->amount - $totalMrWithReturn;
                            $totalBill += $dt->amount;
                            $totalDue += $due_amount;
                            ?>
                            @if ($due_amount > 0)
                                ...
                                <tr class="cartList">
                                    ...
                                    <td>
                                        <input type="hidden" name="mr[invoice_id][]" class="form-control invoice_id"
                                               value="{{ $dt->id }}">
                                        <span class="invoice-no">{{ $dt->voucher_no }}</span>
                                    </td>
