        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title" id="basic-layout-card-center">Edit a Expense type</h4>
                            ...
                            <div class="form-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="select2 form-control @error('root_id') is-invalid @enderror" id="root_id" name="root_id" required>
                                        ...
                                        @foreach($rootTypes as $rootType)
                                            <option value="{{$rootType->id}}" {{ (old('root_id')?old('root_id'):$data->root_id)==$rootType->id?'selected':'' }}>{{$rootType->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('root_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="name" class="form-control @error('name') is-invalid @enderror"
                                           placeholder="Expense Type Name" name="name" value="{{old('name')?old('name'):$data->name}}" />
                                    @error('name')
                                    <div class="help-block text-danger">{{ $message }} </div> @enderror
                                </div>
...
@push('scripts')
    <script>
        ...
        function ajaxSave() {
            ...
            $.ajax({
                url: "{{ route('accounts.expense-types.update',$data->id) }}",
                type: 'post',
                dataType: "json",
                cache: false,
                data: $('form').serialize(),
