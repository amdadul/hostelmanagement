@extends('admin.app')
@section('title') {{ isset($pageTitle)?$pageTitle:'Expense Type' }} @endsection
@push('styles')
    ...
    <div class="row">
        <div class="col-12 text-right">
            <a type="button" class="btn btn-info btn-min-width mr-1 mb-1"
               href="{{route('accounts.expense-types.create')}}"><i
...
@push('scripts')
    ...
    <script>
        $('#expense-type-table').on('click', '.btn-delete[data-remote]', function (e) {
            e.preventDefault();
            $.ajaxSetup({
                headers: {
            ...
            if (confirm('Are you sure you want to delete this row?')) {
                $.ajax({
                    ...
                    $('#expense-type-table').DataTable().draw(false);
