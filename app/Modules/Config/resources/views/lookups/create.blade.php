                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="select2 form-control @error('type') is-invalid @enderror" id="type" name="type" >
                                        ...
                                        @foreach($types as $type)
                                                <option value="{{$type->type}}"  {{(old('type')?old('type'):0)==$type->type?'selected':''}} >{{$type->type}}</option>
                                        @endforeach
                                </select>
                                @error('type')
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="lookup_name" class="form-control @error('lookup_name') is-invalid @enderror"
                                          placeholder="Lookup Name" name="lookup_name" value=" {{old('lookup_name')?old('lookup_name'):''}}" required />
                                @error('lookup_name')
                                <div class="help-block text-danger">{{ $message }} </div> @enderror
                            </div>
...
@push('scripts')
    ...
    function ajaxSave() {
        ...
        $.ajax({
            url: "{{ route('config.lookups.store') }}",
            type: 'post',
            dataType: "json",
            cache: false,
            data: $('form').serialize(),
